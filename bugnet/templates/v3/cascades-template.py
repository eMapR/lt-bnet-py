...
param['target'] = 2015
param['aoi'] = ee.FeatureCollection(ee.FeatureCollection("EPA/Ecoregions/2013/L3").filter(ee.Filter.eq('us_l3name', 'Cascades')).geometry().intersection(ee.FeatureCollection("TIGER/2018/States").filter(ee.Filter.Or(ee.Filter.eq('NAME', 'Oregon'),ee.Filter.eq('NAME', 'Washington'))).geometry(), ee.ErrorMargin(1)))
