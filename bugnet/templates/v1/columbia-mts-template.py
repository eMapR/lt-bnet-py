...
import datetime
param = {}
param['project_name'] = 'columbia-mts-bugnet'
ee.Initialize(project=param['project_name'])
param["platform"] = 'LS'
param['ltstartYear'] = 2000
param['ltendYear'] = 2025
param['target'] = 2025
param['aoi'] = ee.FeatureCollection(ee.FeatureCollection("EPA/Ecoregions/2013/L3").filter(ee.Filter.eq('us_l3name', 'Northern Rockies')).geometry().intersection(ee.FeatureCollection("TIGER/2018/States").filter(ee.Filter.Or(ee.Filter.eq('NAME', 'Oregon'),ee.Filter.eq('NAME', 'Washington'))).geometry(), ee.ErrorMargin(1)))
