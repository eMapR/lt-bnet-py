...
import datetime
param = {}
param['project_name'] = 'coast-range-bugnet'
ee.Initialize(project=param['project_name'])
param["platform"] = 'LS'
param['ltstartYear'] = 2000
param['ltendYear'] = 2025
param['target'] = 2025
param['aoi'] = ee.FeatureCollection(ee.FeatureCollection("EPA/Ecoregions/2013/L3").filter(ee.Filter.eq('na_l3name','Coast Range')).geometry().intersection(ee.FeatureCollection("TIGER/2018/States").filter(ee.Filter.Or(ee.Filter.eq('NAME', 'Oregon'),ee.Filter.eq('NAME', 'Washington'))).geometry(), ee.ErrorMargin(1)))
...
param['subregion']= 'coast-range'
param['sub_region']="coast-range"
...
# SNIC path (configName='option1', no "3") - snic() segments
# LTSDdir+LTSDname, declining_snic() reads snicName. Every real SNIC-path
# config sets LTSDname to fitted_img_p - see docs/parameter_reference_template.py
# and docs/config-layout.md for why this is required here but not on the
# LTSD-path (templates/v3/) templates.
...
param['lt_collection_params'], param['lt_params'] = bnet.build_lt_params(param)
