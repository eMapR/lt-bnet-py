...
param['subregion']= 'northCascades'
param['sub_region']="northCascades"
...
param['decline_thresholds'] = {'tcb': 80, 'tcg': 80, 'tcw': 80} 
param['kmeans_num_sample'] =1000
param['num_of_clusters'] = 3

#polygonization
param['bnet_polygon_mmu'] = 40
...
param['wild_path'] = {"on":0,"path": f"projects/bnet-main/assets/BdyDesg_LSRS_Wilderness","path2":f"projects/north-cascades-bugnet/assets/nps_boundary"} 

#################### automated parameters ################################
param['assetDir'] = f"projects/{param['project_name']}/assets/{param['target']}-v{param['version']}/" 

param['fitted_img_p'] = f"A_{param['platform']}_predictor_fitted_img_{param['composite_params']['end_date'].year-5}_{param['composite_params']['end_date'].year}" # hardcoded -5
...
#decline
param['declineName'] = f"Decline_{param['platform']}_{param['configName']}_{param['target']}_mag{param['decline_thresholds']['tcw']}"
param['Mask'] = ee.Image(f"{param['assetDir']}{param['forestMaskName']}")

#kmeans
param['kmeansNameSample'] =  f"KMeans_{param['platform']}_{param['configName']}_{param['target']}_mag{param['decline_thresholds']['tcw']}_sample" 
param['kmeansName'] = f"KMeans_{param['platform']}_{param['configName']}_{param['target']}_mag{param['decline_thresholds']['tcw']}"
#param['KmeansVector'] = f"KMeans_{param['configName']}_{param['target']}_vector"

#bugnet labeling
param['predicted'] = f"labeled_{param['platform']}_{param['configName']}_{param['target']}_mag{param['decline_thresholds']['tcw']}"

#polygonization
param['bnet_polygonized'] = f"bugnet_{param['platform']}_polygons_{param['target']}_mag{param['decline_thresholds']['tcw']}_{param['bnet_polygon_mmu']}mmu"
param['bnet_buffered_polygons'] = f"bugnet_{param['platform']}_polygons_buffered_{param['target']}_mag{param['decline_thresholds']['tcw']}_{param['bnet_polygon_mmu']}mmu"

#parameters export
param['parameter_file'] = f"{param['project_name']}_{param['platform']}_mag{param['decline_thresholds']['tcw']}_{param['bnet_polygon_mmu']}mmu_parameter_file"

param['outputfile_prefix'] = f"Bugnet_{param['subregion']}_{param['platform']}_v{param['target']}-{param['version']}_Annual_Change" 
